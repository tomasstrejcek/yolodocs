...
import {
  loadSchemaFromIntrospectionUrl,
...
function buildNavigationManifest(
  ...
  if (docsManifest.pages.length > 0) {
    ...
    const folderMap = new Map<string, typeof docsManifest.pages>();
    ...
    const sortPages = (a: { order: number; title: string }, b: { order: number; title: string }) => {
    ...
    const items: Array<{ id: string; name: string; anchor: string; description: string; children?: Array<{ id: string; name: string; anchor: string; description: string }> }> = [];
...
async function runDevServer(buildDir: string): Promise<void> {
  console.log("        Installing dependencies...");
  execSync("npm install", { cwd: buildDir, stdio: "pipe" });
