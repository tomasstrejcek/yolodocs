...
const SCALAR_MOCKS: Record<string, unknown> = {
...
class ExampleContext {
  ...
  private enumMap: Map<string, EnumDefinition>;
  private inputMap: Map<string, InputDefinition>;
  private maxDepth: number;

  constructor(schema: ParsedSchema, maxDepth: number) {
  ...
  private generateMockResponse(typeRef: TypeRef, depth: number): unknown {
    const namedType = unwrapType(typeRef);
  ...
  mockValueForType(typeRef: TypeRef): unknown {
...
function isList(ref: TypeRef): boolean {
  ...
  if (ref.kind === "NON_NULL" && ref.ofType) return isList(ref.ofType);
  return false;
}
